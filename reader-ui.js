...
(function(){
  ...
  const more=button('','acts-more',openActPicker),moreLabel=document.createElement('span'),moreSlot=document.createElement('div');moreLabel.textContent='+';more.append(moreLabel);more.dataset.maxCount='+'+bar.querySelectorAll('button[data-act]').length;more.setAttribute('aria-label','Otwórz katalog prawa');moreSlot.className='acts-more-slot';moreSlot.append(more);wrap.append(moreSlot);
  ...
  function measurePills(){
    ...
    lastCue=cue.amount;if(cue.amount>.65)tailCanSnap=true;
    ...
    if(moreLabel.textContent!==cue.text)moreLabel.textContent=cue.text;more.style.setProperty('--cue-amount',String(cue.amount));more.disabled=cue.amount===0;more.tabIndex=cue.amount===0?-1:0;more.setAttribute('aria-hidden',String(cue.amount===0));
    more.setAttribute('aria-label',cue.count?cue.count+' pozycji po prawej. Wybierz pozycję.':'Wybierz pozycję');releaseTail();
  }
