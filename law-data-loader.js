...
(function(root){
  ...
  function packInfo(packId){if(packId==="documents")return documentManifest?{name:root.__LAW_CONFIG?.documents?.name||"Dokumenty i wzory",acts:Object.keys(documentManifest.acts||{}),mandatory:true}:null;return manifest.packs?.[packId]||null}
  function enabledPackIds(){return Object.entries(manifest.packs||{}).sort(([,a],[,b])=>(a.order||0)-(b.order||0)).filter(([,pack])=>pack.mandatory||pack.acts.some(code=>packages.isEnabled?.(code)!==false)).map(([id])=>id)}
  ...
  async function loadSearch(packId){if(searchCache.has(packId))return searchCache.get(packId);const pack=manifest.packs?.[packId];if(!pack)throw new Error("Brak indeksu pakietu: "+packId);const promise=gunzipJson(pack.search,{packId,kind:"search"}).catch(error=>{searchCache.delete(packId);throw error});searchCache.set(packId,promise);const value=await promise;searchCache.set(packId,value);return value}
