...
assert.equal(w.document.querySelectorAll('#packageList input[data-code]').length,2);
assert.equal(w.document.querySelectorAll('#packageList input[data-package-toggle]').length,2);
const catalogPills=[...w.document.querySelectorAll('#quickbar button')];
assert.equal(w.__POLICE_PACKAGES.setOrder(['alk','uop']),true);
assert.deepEqual([...w.document.querySelectorAll('#quickbar button')],catalogPills.toReversed(),'saving order moves existing pill nodes');
assert.equal(w.__POLICE_PACKAGES.setOrder(['alk','alk']),false,'reject duplicate catalog entries');
assert.equal(w.__POLICE_PACKAGES.setOrder(['uop','alk']),true);
assert.deepEqual(JSON.parse(w.localStorage.getItem('police-law-act-order-v1')),['uop','alk']);
...
assert.equal(query('#searchActFilters [data-code="alk"]'),null,'package-disabled acts are absent from search settings');
...
const originalCapture=w.__READER_STATE.capture,originalToElement=w.__READER_STATE.toElement;let collapseTarget='';
w.__READER_STATE.capture=()=>({articleId:'alk-art-1',id:'alk-art-1',ratio:0});
w.__READER_STATE.toElement=element=>{collapseTarget=element.id};
leftScroll.scrollTop=173;click('.favorites-all-law[data-act="alk"] .favorites-all-law-title');
assert.equal(collapseTarget,'uop-art-315','closing the active law uses the previous visible law’s last favorite');
assert.equal(leftScroll.scrollTop,173);assert.equal(w.__READER_FOLLOW.manual,true);
w.__READER_STATE.capture=originalCapture;w.__READER_STATE.toElement=originalToElement;
click('.favorites-all-law[data-act="alk"] .favorites-all-law-title');
click('.favorites-all-law[data-act="uop"] [data-id="uop-art-315"]');assert.equal(w.__READER_FOLLOW.manual,false,'an article click resumes following after manual section changes');
