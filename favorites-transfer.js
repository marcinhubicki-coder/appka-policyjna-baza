...
(function(){
  ...
  function openDialog(file,items){pendingItems=items;lastFocus=document.activeElement;if(name)name.textContent='Wybrano: '+file.name;settings&&(settings.inert=true);document.body.classList.add('favorites-import-open');dialog.setAttribute('aria-hidden','false');requestAnimationFrame(()=>dialog.querySelector('[data-import-mode="merge"]')?.focus({preventScroll:true}))}
  ...
  function applyImport(mode){if(!pendingItems)return;try{const items=mode==='replace'?pendingItems:merge(normalizeAll(read()).valid,pendingItems);save(items);const count=pendingItems.length;closeDialog();setStatus(`Zaimportowano ${count} ${count===1?'ulubiony artykuł':'ulubionych artykułów'}.`,'success')}catch(_){closeDialog();setStatus('Nie udało się zapisać importowanych ulubionych.','error')}}
