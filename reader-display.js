...
(function(){
  ...
  function show(anchor){
    const pop=document.createElement('div');pop.className='reader-display-options';pop.setAttribute('role','dialog');pop.setAttribute('aria-label','Wyświetlanie');
    const dismiss=()=>{pop.remove();close(pop);anchor.focus({preventScroll:true})};
    for(const [name,key,value] of [['Wyróżnienia','highlights',highlights],['Tryb ciemny','dark',dark]]){const row=document.createElement('label'),label=document.createElement('span'),toggle=document.createElement('input');row.className='search-filter-row';label.textContent=name;toggle.type='checkbox';toggle.checked=value;toggle.dataset.displayOption=key;toggle.onchange=()=>{if(key==='highlights')highlights=toggle.checked;else dark=toggle.checked;apply()};row.append(label,toggle);pop.append(row)}
    document.body.append(pop);open(pop,dismiss);
    const r=anchor.getBoundingClientRect(),width=pop.offsetWidth||200;pop.style.left=Math.max(6,Math.min(innerWidth-width-6,r.left+r.width/2-width/2))+'px';pop.style.top=Math.min(innerHeight-pop.offsetHeight-8,r.bottom+8)+'px';
  }
