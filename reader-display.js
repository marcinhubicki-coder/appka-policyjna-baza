...
(function(){
  ...
  function show(anchor){
    ...
    for(const [name,key,value] of [['Ulubione','favorites',globalThis.__FAVORITES_HIGHLIGHT?.get()??true],['Wyróżnienia','highlights',highlights],['Tryb ciemny','dark',dark]]){const row=document.createElement('button'),label=document.createElement('span'),toggle=document.createElement('span');row.type='button';row.className='favorites-scope-toggle';row.setAttribute('role','switch');row.setAttribute('aria-checked',String(value));row.dataset.displayOption=key;label.className='favorites-scope-info';label.textContent=name;toggle.className='favorites-scope-switch';toggle.setAttribute('aria-hidden','true');row.onclick=()=>{const checked=row.getAttribute('aria-checked')!=='true';row.setAttribute('aria-checked',String(checked));if(key==='favorites')globalThis.__FAVORITES_HIGHLIGHT?.set(checked);else if(key==='highlights')highlights=checked;else dark=checked;apply()};row.append(label,toggle);pop.append(row)}
